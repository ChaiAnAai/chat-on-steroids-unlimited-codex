...
build({ targets: Platform.WINDOWS.createTarget(['dir'], Arch.x64), config: {
  ...
  directories: { output: unifiedSettings ? 'release-unified-settings-review' : upgradeFoundation ? 'release-upgrade-foundation-review' : marketplaceChinese ? 'release-marketplace-chinese-review' : marketplace ? 'release-marketplace-review' : composerFocus ? 'release-composer-review' : responsive ? 'release-responsive-review' : setupRefinement ? 'release-setup-review' : existingProfiles ? 'release-accounts-profiles-review' : 'release-accounts-review' },
  extraMetadata: { version: unifiedSettings ? '2.1.0-accounts-preview.9' : upgradeFoundation ? '2.0.9-accounts-preview.8' : marketplaceChinese ? '2.0.9-accounts-preview.7' : marketplace ? '2.0.9-accounts-preview.6' : composerFocus ? '2.0.9-accounts-preview.5' : responsive ? '2.0.9-accounts-preview.4' : setupRefinement ? '2.0.9-accounts-preview.3' : existingProfiles ? '2.0.9-accounts-preview.2' : '2.0.9-accounts-preview.1' }
