...
let launch: { nonce: string; allowOpen: boolean; work: Promise<void>; finished: boolean } | null = null;
let changed = (): void => {};
...
export function requestChatModels(allowOpen = true): ChatModelCatalog {
  ...
  else if (allowOpen) request.allowOpen = true;
...
export async function startChatModelDiscovery(allowOpen = true): Promise<ChatModelCatalog> {
  ...
  if (!launch || launch.nonce !== nonce || (request!.allowOpen && !launch.allowOpen)) {
    const attempt = { nonce, allowOpen: request!.allowOpen, work: Promise.resolve(), finished: false };
    const work = (async () => {
      try { if (!wake) throw new Error('Model discovery is not ready'); await wake(nonce, attempt.allowOpen); logInfo(`model discovery browser wake completed id=${nonce}`); }
      catch (error) {
