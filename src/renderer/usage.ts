...
function saveFormula(): void {
  try {
    ...
    // Read-only storage still permits an in-memory comparison.
  }
}
...
const featureLabels: Record<string, string> = {
  ...
  paste_text_to_file: 'Pasted text files',
...
function usageHint(node: HTMLElement, text: string): void {
  ...
  const show = () => {
    hide();
    const tip = el('div', 'session-tooltip', node.dataset.usageHint ?? '');
    tip.id = 'usageTooltip';
...
export async function refreshUsage(): Promise<void> {
  ...
  try {
    ...
    for (const [label, number] of [
      [ui().estimatedTokens, value.tokens],
      ['Peak daily tokens', Math.max(0, ...value.days.map((day) => day.tokens))],
      ['Conversations', value.sessions],
      ['Active days', value.days.filter((day) => day.tokens > 0).length]
    ] as const) {
      const item = el('div');
      item.dataset.usageMetric = label;
      usageHint(item, `${Math.round(number).toLocaleString()} ${label.toLowerCase()}`);
      item.append(el('strong', '', count.format(number)), el('span', '', label));
      summary.append(item);
    }
    const limits = $('modelUsage');
    ...
    for (const model of knownModels.filter((item) => !modelRows.some((row) => row.model === item.id))) {
      ...
      row.append(el('strong', '', model.label), el('span', 'muted', ui().usageNotReported));
      limits.append(row);
    }
    for (const entry of [...modelRows, ...value.limits.filter((row) => row.scope !== 'model')]) {
      ...
      if (entry.scope !== 'model') name.append(el('small', 'muted', entry.scope === 'shared' ? ui().usageSharedPool : ui().usageFeatureQuota));
      const detail = el('div');
      detail.append(
        el(
          'b',
          '',
          stale
            ? ui().usageRefreshNeeded
            : entry.remaining !== null
              ? ui().usageRemaining.replace('{value}', entry.remaining.toLocaleString())
              : entry.remainingPercent !== null
                ? ui().usagePercentRemaining.replace('{value}', String(Math.round(entry.remainingPercent)))
                : ui().usageNotReported
        )
      );
      ...
      if (entry.remainingPercent !== null && !stale) {
        const progress = document.createElement('progress');
        progress.max = 100;
        progress.value = entry.remainingPercent;
        progress.setAttribute('aria-label', `${displayName}: ${entry.remainingPercent}% remaining`);
    ...
    if (!modelRows.length) {
      limits.append(el('p', 'muted', 'ChatGPT has not reported per-model message balances. Shared usage and feature quotas do not establish a model-specific balance.'));
    }
    const totalCost = el('div');
    totalCost.append(el('strong', '', '—'), el('span', '', ui().estimatedEquivalent));
    totalCost.id = 'usageTotalCost';
    usageHint(totalCost, '');
    summary.prepend(totalCost);
    paintRates();
    paintCost();
    status.textContent = ui().usageFresh;
  } finally {
  ...
  for (const model of [...new Set(snapshot.models.map(row => row.model))].sort()) {
    ...
    text.append(
      ...
      el(
        'em',
        '',
        Object.hasOwn(DEFAULT_USAGE_FORMULA.rates, model)
          ? 'USD / 1M cached input · editable official baseline, checked 5 September 2026'
          : 'USD / 1M cached input · enter a verified comparison rate'
      )
    );
    ...
    input.min = '0';
    input.step = '0.01';
    input.placeholder = ui().usageUnknownRate;
    input.value = formula.rates[model]?.toString() ?? '';
...
function paintCost(): void {
  ...
  for (const [label, number] of [[ui().estimatedTokens, total.tokens], ['Peak daily tokens', Math.max(0, ...daily.map(day => day.tokens))]] as const) {
    const item = [...$('usageSummary').children].find(node => (node as HTMLElement).dataset.usageMetric === label) as HTMLElement | undefined;
    if (item) {
      item.querySelector('strong')!.textContent = count.format(number);
      item.dataset.usageHint = `${Math.round(number).toLocaleString()} ${label.toLowerCase()}`;
    }
  }
  const heat = $('usageHeatmap');
  heat.replaceChildren();
  const byDay = new Map(daily.map(day => [day.date, day.tokens]));
  ...
  for (let ago = 363; ago >= 0; ago--) {
    ...
    cell.dataset.level = String(tokens ? Math.max(1, Math.ceil(tokens / peak * 4)) : 0);
    const hint = `${key}: ${Math.round(tokens).toLocaleString()} estimated tokens`;
    usageHint(cell, hint);
  ...
  $('usageCost').textContent = `${costText(total)} ${ui().usageComparisonNotBill}`;
  const modelTable = el('table', 'usage-table');
  const modelHead = el('tr');
  for (const title of [ui().recordedModelEffort, ui().estimatedTokens, ui().estimatedEquivalent]) modelHead.append(el('th', '', title));
  modelTable.append(modelHead);
  for (const entry of snapshot.models) {
    const estimate = usageEstimate([entry], formula);
    const row = el('tr');
    row.append(
      el('td', '', `${entry.model} · ${entry.reasoningEffort ?? 'effort unknown'}${entry.assumed ? ' (assumed)' : ''}`),
      el('td', '', Math.round(estimate.tokens).toLocaleString()),
      el('td', '', estimate.unpricedTokens ? ui().usageMissingRate : money.format(estimate.cost))
    );
    modelTable.append(row);
  }
  const table = el('table', 'usage-table');
  const head = el('tr');
  head.append(el('th', '', ui().day), el('th', '', ui().estimatedTokens), el('th', '', ui().cachedMultiplier.replace('{multiplier}', String(formula.multiplier))));
  table.append(head);
  for (const day of [...daily].reverse()) {
    const row = el('tr');
    row.append(el('td', '', day.date), el('td', '', Math.round(day.tokens).toLocaleString()), el('td', '', costText(day)));
    table.append(row);
  }
  if (!snapshot.days.length) {
    const row = el('tr');
    const cell = el('td', 'muted', ui().noRecordedCallsYet);
  ...
  try {
    ...
    if (saved && Number.isFinite(saved.divisor) && saved.divisor > 0 && Number.isFinite(saved.multiplier) && saved.multiplier >= 0 && saved.rates && typeof saved.rates === 'object' && !Array.isArray(saved.rates)) {
      formula = {
        ...
        rates: {
          ...DEFAULT_USAGE_FORMULA.rates,
          ...Object.fromEntries(Object.entries(saved.rates).filter(([key, value]) => key.length <= 100 && (value === null || typeof value === 'number' && Number.isFinite(value) && value >= 0)))
        } as UsageFormula['rates']
      };
    }
  } catch {
    // Invalid display preferences use the documented default.
  }
  for (const [key, id] of [['divisor', 'usageDivisor'], ['multiplier', 'usageMultiplier']] as const) {
    const input = $<HTMLInputElement>(id);
    input.value = String(formula[key]);
    input.addEventListener('input', () => {
      if (input.value !== '' && input.validity.valid && Number.isFinite(input.valueAsNumber)) {
        formula[key] = input.valueAsNumber;
        saveFormula();
        paintCost();
      }
    });
